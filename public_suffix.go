...
var publicSuffix = make(hash)

func init() {
	r := bufio.NewReader(strings.NewReader(public_suffix_list_dat))
	for {
		...
		for i := len(ss) - 1; i >= 0; i-- { // construct a reversed suffix hash chain
			suffix := ss[i]
			subHash = getSubHash(subHash, suffix)
		}
	}

}

func getSubHash(h hash, name string) hash {
	if hh, ok := h[name].(hash); ok {
		return hh
	}
	hh := make(hash)
	h[name] = hh
